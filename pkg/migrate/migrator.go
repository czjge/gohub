...
type Migration struct {
	...
	Migration string `gorm:"type:varchar(250);not null;unique;default:'';"`
	Batch     int
}
...
func NewMigrator() *Migrator {
	...
	// DB := database.DB().Set("gorm:table_options", "ENGINE=InnoDB")
...
func (migrator *Migrator) readAllMigrationFiles() []MigrationFile {
	...
	for _, f := range files {
		...
		// 去除文件后缀 .go
		fileName := file.FileNameWithoutExtention(f.Name())
...
func (migrator *Migrator) runUpMigration(mfile MigrationFile, batch int) {
	...
	if mfile.Up != nil {
		...
		// 执行 up 方法
		mfile.Up(database.DB().Set("gorm:table_options", "ENGINE=InnoDB").Migrator(), database.SQLDB())
		// 提示已迁移了哪个文件
...
func (migrator *Migrator) rollbackMigrations(migrations []Migration) bool {
	...
	for _, _migration := range migrations {
		...
		if mfile.Down != nil {
			mfile.Down(database.DB().Set("gorm:table_options", "ENGINE=InnoDB").Migrator(), database.SQLDB())
		}
